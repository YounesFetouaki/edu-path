...
Experimental features include an asynchronous event bus powered by \textbf{RabbitMQ} (demonstrated in \texttt{RabbitMQ\_Demo}), enabling decoupled communication between the \textit{LMSConnector} (producer) and \textit{PrepaData} (consumer). This ensures that heavy data ingestion loads do not degrade the performance of the user-facing APIs.
...
\begin{itemize}
    \item \textbf{Temporal Aggregation}: Logs were aggregated into weekly feature vectors. Features included \texttt{weekly\_login\_count}, \texttt{avg\_session\_duration}, \texttt{forum\_read\_count}, \texttt{forum\_post\_count}, and \texttt{quiz\_avg\_score}.
...
\begin{figure}[H]
    ...
    \caption{Visual representation of the Machine Learning Pipeline in EduPath-MS, detailing the flow from LMS data ingestion to Risk Prediction.}
...
The analysis reveals that \texttt{quiz\_submission\_latency} (the time between an assignment opening and the student submitting) is the single most predictive feature (32.4\%). Students who submit assignments minutes before the deadline—or late—are significantly more likely to fail. \texttt{quiz\_avg\_score} (22.1\%) and \texttt{total\_login\_days} (15.5\%) follow, confirming that consistent engagement is as important as raw competence.
...
\begin{figure}[H]
    ...
    \caption{ROC Curve for the PathPredictor XGBoost Model, demonstrating a high Area Under Curve (AUC) of 0.91, indicating strong separability between Pass and Fail classes.}
    \label{fig:roc_curve}
\end{figure}
...
\begin{enumerate}
    \item \textbf{Cluster 0 ("At Risk" - 35\% of population)}: Characterized by low \texttt{total\_time} ($z=-1.2$), high \texttt{latency} ($z=+0.9$), and low \texttt{quiz\_score} ($z=-0.8$). This group aligns closely with the "Fail" class predictions.
    \item \textbf{Cluster 1 ("Standard" - 45\% of population)}: Average metrics across the board. \texttt{quiz\_score} near 0.0 (mean). This group requires monitoring but not immediate intervention.
    \item \textbf{Cluster 2 ("High Achiever" - 20\% of population)}: Very high \texttt{quiz\_score} ($z=+1.5$), high \texttt{forum\_post\_count} ($z=+1.2$). Interestingly, their \texttt{total\_time} was not the highest—indicating efficiency.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Profiling}: \textit{StudentProfiler} detects the drop in \texttt{login\_regularity}. His cluster membership probability shifts: Cluster 1 (80\%) $\to$ Cluster 0 (65\%).
...
\begin{enumerate}
    ...
    \item \textbf{AI Correction}: \textit{StudentProfiler} (K-Means) groups her into Cluster 2 because her \texttt{quiz\_score} and \texttt{efficiency} (Score/Time) are outliers.
