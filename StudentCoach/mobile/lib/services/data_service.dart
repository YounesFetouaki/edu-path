...
class DataService {
  ...
  String get chatUrl {
    if (kIsWeb) return 'http://localhost:8001/chat';
    if (Platform.isAndroid) return 'http://10.0.2.2:8001/chat';
  ...
  Future<List<dynamic>> fetchCourseStructure(int courseId) async {
    final response = await http.get(Uri.parse('$baseUrl/lms/courses/$courseId/structure'));
    if (response.statusCode == 200) {
      return json.decode(response.body);
    } else {
  ...
  Future<List<dynamic>> fetchAssignments(int studentId) async {
    final response = await http.get(Uri.parse('$baseUrl/lms/assignments/$studentId'));
    if (response.statusCode == 200) {
      return json.decode(response.body);
    } else {
  ...
  Future<List<dynamic>> fetchGrades(int studentId) async {
    final response = await http.get(Uri.parse('$baseUrl/lms/grades/$studentId'));
    if (response.statusCode == 200) {
      return json.decode(response.body);
    } else {
  ...
  // NEW: Submit Grade
  Future<void> submitQuizResult(int studentId, int quizId, int score, int maxScore) async {
    await http.post(
    ...
    try {
        ...
        if (response.statusCode == 200) {
          return json.decode(response.body); 
             ...
             return [
                 {'question': 'Offline: What is $topic?', 'options': ['Tech', 'Food', 'Car', 'Pet'], 'correct': 0}
             ];
        }
    } catch(e) {
        print('Quiz Gen Network Error: $e');
        return [
             {'question': 'Network Err: What is $topic?', 'options': ['Tech', 'Food', 'Car', 'Pet'], 'correct': 0}
        ];
    }
  }
}
