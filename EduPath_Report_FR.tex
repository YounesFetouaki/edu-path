...
Les fonctionnalités expérimentales incluent un bus d'événements asynchrone propulsé par \textbf{RabbitMQ} (démontré dans `RabbitMQ_Demo`), permettant une communication découplée entre le \textit{LMSConnector} (producteur) et \textit{PrepaData} (consommateur). Cela garantit que les lourdes charges d'ingestion de données ne dégradent pas les performances des API orientées utilisateur.
...
\begin{itemize}
    \item \textbf{Agrégation Temporelle} : Les journaux ont été agrégés en vecteurs de caractéristiques hebdomadaires. Les caractéristiques incluaient `weekly_login_count` (nombre de connexions hebdo), `avg_session_duration` (durée moy. session), `forum_read_count` (lectures forum), `forum_post_count` (messages forum) et `quiz_avg_score` (score moy. quiz).
...
L'analyse révèle que `quiz_submission_latency` (le temps entre l'ouverture d'un devoir et sa soumission par l'étudiant) est la caractéristique la plus prédictive (32,4 \%). Les étudiants qui soumettent des devoirs quelques minutes avant la date limite — ou en retard — sont nettement plus susceptibles d'échouer. `quiz_avg_score` (22,1 \%) et `total_login_days` (15,5 \%) suivent, confirmant qu'un engagement constant est aussi important que la compétence brute.
...
\begin{enumerate}
    \item \textbf{Cluster 0 ("À Risque" - 35 \% de la population)} : Caractérisé par un faible `total_time` ($z=-1,2$), une latence élevée `latency` ($z=+0,9$) et un faible `quiz_score` ($z=-0,8$). Ce groupe s'aligne étroitement avec les prédictions de la classe "Échec".
    \item \textbf{Cluster 1 ("Standard" - 45 \% de la population)} : Métriques moyennes dans l'ensemble. `quiz_score` proche de 0,0 (moyenne). Ce groupe nécessite une surveillance mais pas d'intervention immédiate.
    \item \textbf{Cluster 2 ("Haute Performance" - 20 \% de la population)} : Très haut `quiz_score` ($z=+1,5$), nombre élevé de messages forum `forum_post_count` ($z=+1,2$). Fait intéressant, leur `total_time` n'était pas le plus élevé — indiquant une efficacité.
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Optimisation des Ressources} : Le `RecoBuilder` garantit que les ressources de tutorat humain coûteuses sont allouées aux étudiants qui en ont le plus besoin (Haut Risque), tandis que les étudiants Standard sont soutenus via des recommandations de contenu automatisées.
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Profilage} : \textit{StudentProfiler} détecte la baisse de `login_regularity`. Sa probabilité d'appartenance au cluster change : Cluster 1 (80 \%) $\to$ Cluster 0 (65 \%).
...
\begin{enumerate}
    ...
    \item \textbf{Correction IA} : \textit{StudentProfiler} (K-Means) la classe dans le Cluster 2 car son `quiz_score` et son `efficiency` (Score/Temps) sont des valeurs aberrantes.
